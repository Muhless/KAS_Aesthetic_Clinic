...
<div class="bg-white rounded-lg border border-gray-200 overflow-hidden hover:border-gray-300 transition-colors">
    <!-- Foto Dokter -->
    <div class="relative h-48 bg-gray-50 cursor-pointer" onclick="window.location.href='{{ route('dokter.detail', $dokter['id']) }}'">
    ...
    <div class="p-5 cursor-pointer hover:bg-gray-50 transition-colors"
        ...
        @if($dokter->jadwalHariIni())
            <div class="mt-3 inline-flex items-center gap-2 px-3 py-1.5 bg-green-50 text-green-700 rounded-full text-sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <span>Praktek Hari Ini: {{ $dokter->jadwalHariIni()->jam_mulai }} - {{ $dokter->jadwalHariIni()->jam_selesai }}</span>
            </div>
        @else
