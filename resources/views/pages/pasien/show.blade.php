        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-gradient-to-r from-primary-600 to-primary-500 px-6 py-5">
                <div class="flex items-center gap-4">
                    ...
                    <button type="button" @click="editPasien({{ $pasien->id }})"
                    ...
                    <button type="button" onclick="confirmDelete({{ $pasien->id }}, '{{ $pasien->nama }}')"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-red-500/80 text-white text-sm rounded-lg transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                            ...
                            <div class="flex items-center justify-between px-5 py-3.5 bg-gray-50 border-b border-gray-100">
                                ...
                                <div class="flex items-center gap-2">
                                    @if ($pelayanan->reservasi_id)
                                        <span
                                            class="text-xs px-2 py-1 rounded-full bg-violet-50 text-violet-700 font-medium">Reservasi</span>
                                    @else
                                        <span
                                            class="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-600 font-medium">Walk-in</span>
                                    @endif
                                    @if ($pelayanan->status == 'selesai')
                                        <span
                                            class="text-xs px-2 py-1 rounded-full bg-green-50 text-green-700 font-medium">Selesai</span>
                                    @elseif ($pelayanan->status == 'dipanggil')
                                        <span
                                            class="text-xs px-2 py-1 rounded-full bg-blue-50 text-blue-700 font-medium">Dipanggil</span>
                                    @else
                                        <span
                                            class="text-xs px-2 py-1 rounded-full bg-yellow-50 text-yellow-700 font-medium">Menunggu</span>
                                    @endif
                            ...
                            <div class="px-5 py-4">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    {{-- Keluhan --}}
                                    <div>
                                        <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Keluhan
                                        </p>
                                    ...
                                    @if ($pelayanan->pemeriksaan)
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-700">
                                                {{ $pelayanan->pemeriksaan->treatment->nama ?? '—' }}</p>
                                        </div>

                                        <div>
                                            <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">
                                                Diagnosa</p>
                                            <p class="text-sm text-gray-700">{{ $pelayanan->pemeriksaan->diagnosa ?? '—' }}
                                            </p>
                                        </div>
                                        ...
                                        <div>
                                            <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">
                                                Tindakan</p>
                                            <p class="text-sm text-gray-700">{{ $pelayanan->pemeriksaan->tindakan ?? '—' }}
                                            </p>
                                        </div>
                                        ...
                                        @if ($pelayanan->pemeriksaan->catatan)
                                            <div>
                                                <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">
                                                    Catatan</p>
                                                <p class="text-sm text-gray-700">{{ $pelayanan->pemeriksaan->catatan }}
                                                </p>
                                            </div>
                                        @endif
                                    @else
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if ($pelayanan->pembayaran->metode_bayar)
                                                <span class="text-xs text-gray-400">via
                                                    {{ ucfirst($pelayanan->pembayaran->metode_bayar) }}</span>
                                            @endif
                                        </div>
                                        @if ($pelayanan->pembayaran->status == 'lunas')
                                            <span
                                                class="text-xs px-2.5 py-1 rounded-full bg-green-50 text-green-700 font-medium">Lunas</span>
                                        @else
                                            <span
                                                class="text-xs px-2.5 py-1 rounded-full bg-red-50 text-red-600 font-medium">Belum
                                                Bayar</span>
                                        @endif
                                    </div>
                                @endif
            ...
            @else
                <div
                    ...
                    <div class="w-14 h-14 rounded-full bg-primary-50 flex items-center justify-center mb-3">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-7 h-7 text-primary-300" fill="none"
