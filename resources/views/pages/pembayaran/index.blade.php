...
@section('content')
    <div class="p-6" x-data="{ open: false }">
        ...
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-bold text-primary-400">Pelayanan</h1>
        ...
        <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
                ...
                <p class="text-3xl font-bold text-yellow-500">{{ $pelayanans->where('status', 'menunggu')->count() }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
                <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Dipanggil</p>
                <p class="text-3xl font-bold text-blue-500">{{ $pelayanans->where('status', 'dipanggil')->count() }}</p>
            </div>
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
                <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Selesai</p>
                <p class="text-3xl font-bold text-green-500">{{ $pelayanans->where('status', 'selesai')->count() }}</p>
            </div>
        </div>
        ...
        @else
            <div class="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col items-center justify-center py-24 px-6">
                ...
                <h3 class="text-lg font-semibold text-gray-700 mb-1">Belum ada pelayanan hari ini</h3>
                <p class="text-sm text-gray-400 text-center max-w-sm mb-6">
                    Tambahkan pasien walk-in atau konfirmasi reservasi untuk memulai pelayanan.
                </p>
                <button @click="open = true"
            ...
            <div class="relative z-10 w-full max-w-lg mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden"
                ...
                <div class="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-primary-600 to-primary-500">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-lg font-semibold text-white tracking-wide">Tambah Walk-in</h2>
                    </div>
                ...
                <form action="{{ route('pelayanan.store') }}" method="POST">
                    ...
                    <div class="px-6 py-5 space-y-4">
                        ...
                        {{-- Dokter --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Dokter</label>
                        ...
                        ">
                            ...
                            <div class="relative">
                                ...
                                <input x-ref="tanggalPelayanan" type="text" name="tanggal"
                                    placeholder="Pilih tanggal" readonly
                                    class="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent transition placeholder-gray-400 cursor-pointer @error('tanggal') border-red-400 @enderror">
                            </div>
                            @error('tanggal')
                                <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                            @enderror
